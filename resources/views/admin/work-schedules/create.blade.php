    ...
    <x-slot name="header">
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                ...
                <p class="text-xs text-gray-500 mt-1">Atur jam masuk, jam pulang, toleransi keterlambatan, dan hari kerja aktif.</p>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-200 p-6 sm:p-8">
                <form action="{{ route('admin.work-schedules.store') }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama" id="nama" value="{{ old('nama') }}" required placeholder="Contoh: Jadwal Reguler Dinas (Senin - Jumat)" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('nama') border-red-500 @enderror">
                        @error('nama')
                    ...
                    <div class="grid grid-cols-1 sm:grid-cols-2 gap-4">
                        <div>
                            ...
                            <input type="time" name="jam_masuk" id="jam_masuk" value="{{ old('jam_masuk', '07:30') }}" required class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('jam_masuk') border-red-500 @enderror">
                            @error('jam_masuk')
                                <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
                        ...
                        <div>
                            ...
                            <input type="time" name="jam_pulang" id="jam_pulang" value="{{ old('jam_pulang', '16:00') }}" required class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('jam_pulang') border-red-500 @enderror">
                            @error('jam_pulang')
                                <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                            @enderror
                        </div>
                    ...
                    <div>
                        ...
                        <input type="number" min="0" max="120" name="toleransi_terlambat" id="toleransi_terlambat" value="{{ old('toleransi_terlambat', 15) }}" required class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('toleransi_terlambat') border-red-500 @enderror">
                        <p class="mt-1 text-xs text-gray-500">Contoh: Jika jam masuk 07:30 dengan toleransi 15 menit, maka absensi hingga 07:45 dianggap Hadir, setelah itu dianggap Terlambat.</p>
                        ...
                        <div class="grid grid-cols-2 sm:grid-cols-4 gap-3 bg-gray-50 p-4 rounded-xl border border-gray-200">
                            ...
                            @foreach($dayList as $key => $label)
                                <label class="flex items-center space-x-2 text-sm text-gray-700 cursor-pointer">
                                    <input type="checkbox" name="{{ $key }}" value="1" {{ old($key, in_array($key, $defaultDays) ? '1' : '0') == '1' ? 'checked' : '' }} class="rounded border-gray-300 text-indigo-600 shadow-sm focus:ring-indigo-500">
                                    <span>{{ $label }}</span>
                                </label>
                            @endforeach
                        </div>
