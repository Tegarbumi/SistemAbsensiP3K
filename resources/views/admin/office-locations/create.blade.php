        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    {{ __('Tambah Lokasi Kantor Absensi') }}
                </h2>
                <p class="text-xs text-gray-500 mt-1">Tentukan titik koordinat dan radius area absensi yang valid.</p>
            </div>
    ...
    <div class="py-8">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-200 p-6 sm:p-8">
                <form action="{{ route('admin.office-locations.store') }}" method="POST" id="officeLocationForm">
                    @csrf
                    ...
                    <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
                        ...
                        <div class="lg:col-span-5 space-y-5">
                            ...
                            <div>
                                ...
                                <input type="text" name="nama" id="nama" value="{{ old('nama') }}" required placeholder="Contoh: Kantor Dinas Kearsipan dan Perpustakaan" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('nama') border-red-500 @enderror">
                                @error('nama')
                            ...
                            <div>
                                ...
                                <textarea name="alamat" id="alamat" rows="2" placeholder="Jl. MT Haryono No. 9, Subang" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('alamat') border-red-500 @enderror">{{ old('alamat') }}</textarea>
                                @error('alamat')
                                    <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                            ...
                            <div class="grid grid-cols-2 gap-4">
                                <div>
                                    ...
                                    <input type="number" step="0.0000001" name="latitude" id="latitude" value="{{ old('latitude', $defaultCoords['lat']) }}" required class="mt-1 block w-full rounded-lg border-gray-300 font-mono shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('latitude') border-red-500 @enderror">
                                    @error('latitude')
                                        <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                                    @enderror
                                </div>
                                <div>
                                    ...
                                    <input type="number" step="0.0000001" name="longitude" id="longitude" value="{{ old('longitude', $defaultCoords['lng']) }}" required class="mt-1 block w-full rounded-lg border-gray-300 font-mono shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('longitude') border-red-500 @enderror">
                                    @error('longitude')
                                        <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                                    @enderror
                                </div>
                            ...
                            <div>
                                <div class="flex justify-between items-center">
                                    ...
                                    <span id="radiusDisplay" class="text-xs font-bold text-indigo-600 bg-indigo-50 px-2 py-0.5 rounded-full">
                                        {{ old('radius_meter', $defaultCoords['radius']) }} meter
                                    </span>
                                </div>
                                <input type="number" min="10" max="5000" name="radius_meter" id="radius_meter" value="{{ old('radius_meter', $defaultCoords['radius']) }}" required class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('radius_meter') border-red-500 @enderror">
                                <p class="mt-1 text-xs text-gray-500">Jarak radius batas toleransi PPPK dapat melakukan check-in / check-out (biasanya 50 - 150 meter).</p>
                                @error('radius_meter')
                                    <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                                @enderror
                            </div>
                        ...
                        <div class="lg:col-span-7 space-y-3">
                            <div class="flex items-center justify-between">
                                <h3 class="text-sm font-bold text-gray-900 uppercase tracking-wider">
                                    Tentukan Titik Koordinat di Peta
                                </h3>
                            ...
                            <p class="text-xs text-gray-500">
                                Geser penanda (marker) merah atau klik di peta untuk menentukan koordinat kantor. Lingkaran biru menandakan batas radius absensi.
                            </p>
                            ...
                            <div class="p-3 bg-gray-50 rounded-lg text-xs text-gray-600 space-y-1">
                                <div class="font-medium text-gray-700">Panduan Penentuan Titik:</div>
                                <div>- Pastikan titik berada tepat di area bangunan kantor Dinas Kearsipan dan Perpustakaan Kab. Subang.</div>
                                <div>- Atur radius agar mencakup seluruh area gerbang dan gedung kantor.</div>
                            </div>
                        </div>
                    </div>
