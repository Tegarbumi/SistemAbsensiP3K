        ...
        <div class="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
            <div>
                <h2 class="font-semibold text-xl text-gray-800 leading-tight">
                    {{ __('Edit Unit / Bagian Kerja') }}
                </h2>
                <p class="text-xs text-gray-500 mt-1">{{ $unit->nama }} ({{ $unit->kode }})</p>
            </div>
            ...
            <div class="bg-white overflow-hidden shadow-sm sm:rounded-xl border border-gray-200 p-6 sm:p-8">
                <form action="{{ route('admin.units.update', $unit) }}" method="POST" class="space-y-6">
                    ...
                    <div>
                        ...
                        <input type="text" name="kode" id="kode" value="{{ old('kode', $unit->kode) }}" required class="mt-1 block w-full rounded-lg border-gray-300 uppercase shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('kode') border-red-500 @enderror">
                    ...
                    <div>
                        ...
                        <input type="text" name="nama" id="nama" value="{{ old('nama', $unit->nama) }}" required class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('nama') border-red-500 @enderror">
                        @error('nama')
                            <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <textarea name="alamat" id="alamat" rows="3" class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('alamat') border-red-500 @enderror">{{ old('alamat', $unit->alamat) }}</textarea>
                        @error('alamat')
                            <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                        @enderror
                    </div>
                    ...
                    <div>
                        ...
                        <select name="status" id="status" required class="mt-1 block w-full rounded-lg border-gray-300 shadow-sm text-sm focus:border-indigo-500 focus:ring-indigo-500 @error('status') border-red-500 @enderror">
                            <option value="1" {{ old('status', $unit->status ? '1' : '0') === '1' ? 'selected' : '' }}>Aktif</option>
                            <option value="0" {{ old('status', $unit->status ? '1' : '0') === '0' ? 'selected' : '' }}>Nonaktif</option>
                        </select>
                        @error('status')
                            <p class="mt-1 text-xs text-red-600 font-medium">{{ $message }}</p>
                        @enderror
